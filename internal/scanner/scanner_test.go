...
func TestScanner(t *testing.T) {
	...
	// Test List
	list, hasMore := s.ListByMonth(1, 10, "")
	if len(list) != 3 {
		t.Errorf("Expected 3 markdown files, got %d", len(list))
	}
	if hasMore {
		t.Error("Expected a partial page to be final")
	}
	if list[0].Content == "" {
...
func TestScanner_Pagination(t *testing.T) {
	...
	// Test page < 1
	entries, hasMore := s.ListByMonth(0, 5, "")
	if len(entries) != 5 || !hasMore {
		t.Error("Should default to page 1 if page < 1")
	}

	// Test the default page size and a final partial page.
	entries, hasMore = s.ListByMonth(1, 0, "")
	if len(entries) != 15 || !hasMore {
		t.Fatalf("Expected 15 entries and another page, got %d, %v", len(entries), hasMore)
	}
	entries, hasMore = s.ListByMonth(2, 15, "")
	if len(entries) != 1 || hasMore {
		t.Fatalf("Expected one final entry, got %d, %v", len(entries), hasMore)
	}

	// Test specific page/limit
	entries, hasMore = s.ListByMonth(2, 3, "")
	if len(entries) != 3 || !hasMore {
		t.Errorf("Expected 3 files and another page, got %d, %v", len(entries), hasMore)
	}

	// Test out of bounds
	entries, hasMore = s.ListByMonth(10, 10, "")
	if len(entries) != 0 || hasMore {
		t.Error("Should return empty for page out of range")
	}

	// Exactly one full page must be known to be final.
	if err := os.Remove(filepath.Join(tempDir, "file"+string(rune('a'+16))+".md")); err != nil {
		t.Fatal(err)
	}
	entries, hasMore = s.ListByMonth(1, 15, "")
	if len(entries) != 15 || hasMore {
		t.Fatalf("Expected an exact final page, got %d, %v", len(entries), hasMore)
	}
}
